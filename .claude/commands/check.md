...
2. テストクラスの欠落検査（`verify-tests.ps1`）— `src/LocalRagApplication/` の各クラスに対応するテストクラスが存在するか、無い場合は `tests/LocalRagApplication.Tests/no-test-required.md` に理由付きで除外宣言されているかを確認します。あわせて、`tests/LocalRagApplication.Tests/` の `.cs` が csproj の `<Compile Include>` に登録されているかも確認します
